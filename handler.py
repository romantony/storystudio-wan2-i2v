...
# Resolution mapping - I2V supports both 480p and 720p
RESOLUTION_MAP = {
    "480p": "832*480",
    "720p": "1280*720"
}
...
def generate_video(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Expected job input:
    {
        ...
        "sample_steps": 15-50 (default 30),
        "frame_num": number of frames (default 81 = ~5 sec at 16fps)
    }
    ...
    try:
        ...
        frame_num = job_input.get("frame_num", 81)  # ~5 seconds at 16fps
        ...
        if not (17 <= frame_num <= 161):
            return {"error": "frame_num must be between 17 and 161 (1-10 seconds at 16fps)"}
...
''')
            ...
            # Build command using wrapper - I2V task
            cmd = [
                "python", "-u",
                str(wrapper_script),
                "--task", "i2v-A14B",
            ...
            # Run generation
            print(f"Starting I2V generation: {resolution}, {sample_steps} steps, {frame_num} frames")
            ...
            # Find generated video - I2V outputs with i2v-A14B prefix
            video_files = list(Path(WAN_DIR).glob("i2v-A14B_*.mp4"))
            if not video_files:
                # Fallback: check for any mp4 files
                video_files = list(Path(WAN_DIR).glob("*.mp4"))
...
print("Initializing Wan2.2 I2V handler...")
